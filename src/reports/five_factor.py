...
def format_five_factor_alert(signals: list[dict[str, Any]]) -> str | None:
    ...
    for signal in sorted(signals, key=lambda item: -abs(int(item["score"]))):
        block = [
            "```",
            f"{signal['symbol']} | {signal['verdict']} | Score {int(signal['score']):+d}",
            f"Timeframe: {signal.get('timeframe', 'n/a')}",
            f"Current: {format_price(_get_float(signal, 'current_price'))}",
            f"Funding: {_get_float(signal, 'funding_rate'):+.4%}",
            f"OI: {_get_float(signal, 'oi_change_pct'):+.2f}%",
            f"CVD: {_get_float(signal, 'cvd'):+.2f}",
            f"LSR: {_get_float(signal, 'lsr_global'):.2f}",
        ]
        patterns = signal.get("patterns", [])
        if isinstance(patterns, list) and patterns:
            block.append(f"Patterns: {', '.join(str(pattern) for pattern in patterns)}")
...
def _format_trade_plan(trade_plan: dict[str, Any]) -> list[str]:
    ...
    entry_price = _get_float(trade_plan, "entry_price")
    ...
    if min(entry_price, stop_loss, take_profit_1, take_profit_2) <= 0:
        return []
    return [
        f"Entry MARKET @ {format_price(entry_price)}",
        f"SL {format_price(stop_loss)}",
        f"TP {format_price(take_profit_1)} / {format_price(take_profit_2)}",
        f"RR {risk_reward:.1f}",
    ]
